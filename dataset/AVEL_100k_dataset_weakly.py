...
class VGGSoundAVELDatasetWeakly(Dataset):
    # for AVEL task
    def __init__(self, split='train'):
        super(VGGSoundAVELDatasetWeakly, self).__init__()
        self.split = split
    ...
    # for train
    def _obtain_category_label(self, category, event_label=None):
        class_id = self.category_dirt[category]
        category_num = len(self.category_dirt)
    ...
    def __getitem__(self, index):
        ...
        if self.split != 'train':
            # test
            avel_labels = self._obtain_avel_label(event_label, class_label)

        else:
            # train
            avel_labels = self._obtain_category_label(category, event_label)
        ...
        avel_labels = torch.from_numpy(avel_labels)

        audio_fea = torch.from_numpy(audio_fea)

        return total_img, audio_fea, avel_labels, video_id
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(
        VGGSoundAVELDatasetWeakly(split='train'),
        batch_size=1,
        shuffle=False,
        num_workers=1,
        pin_memory=False
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        # if n_iter==1:
        #     break
        total_img, audio_fea, avel_labels, video_name = batch_data
        # print(total_img.shape)
        print(avel_labels.shape)
