...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(
            AVEDataset('/AVE_data/', split='train'), # split= val, train, test
            batch_size=1,
            shuffle=False,
            num_workers=1,
            pin_memory=False
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            ...
            s2_save_dir = "/AVE_data/swinv2_large_feat_s2"
            s3_save_dir = "/AVE_data/swinv2_large_feat_s3"
            s4_save_dir = "/AVE_data/swinv2_large_feat_s4"

            s2_outfile = os.path.join(s2_save_dir, video_name[0] + '.npy')
            s3_outfile = os.path.join(s3_save_dir, video_name[0] + '.npy')
            s4_outfile = os.path.join(s4_save_dir, video_name[0] + '.npy')

            x = model.patch_embed(visual_feat)
            ...
            for layer in model.layers:
                for blk in layer.blocks:
                    x = x + blk.drop_path1(blk.norm1(blk._attn(x)))
                    x = x + blk.drop_path2(blk.norm2(blk.mlp(x)))
                nums_x.append(x)
                x = layer.downsample(x)

            if os.path.exists(s2_outfile):
                print("文件已经存在")
            else:
                s2_image_features = nums_x[1].cpu().numpy()
                np.save(s2_outfile, s2_image_features)

            if os.path.exists(s3_outfile):
                print("文件已经存在")
            else:
                s3_image_features = nums_x[2].cpu().numpy()
                np.save(s3_outfile, s3_image_features)

            if os.path.exists(s4_outfile):
                print("文件已经存在")
            else:
                s4_image_features = nums_x[3].cpu().numpy()
                np.save(s4_outfile, s4_image_features)
