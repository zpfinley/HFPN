...
class WeaklyLocalizationModule(nn.Module):
    ...
    def forward(self, fused_content):
        ...
        max_fused_content = fused_content.mean(1)
        ...
        # print(event_scores.shape) [bz, 29]
...
class weak_main_model(nn.Module):
    ...
    def forward(self, visual_feature, audio):
        ...
        score = self.predict_fc(video_query_output.transpose(1, 0))
        # print(score.shape)
        out = score.permute(0, 2, 1) # [bz, 29, T]
        out = nn.MaxPool1d(out.size(2))(out).view(out.size(0), -1)

        out = F.softmax(out, dim=-1) # [bz, 29]
